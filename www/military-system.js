...
function processArmsPayment(sellerNation, buyerNation, price, meta) {
  ...
  // Buyer pays from treasury; seller receives treasury income.
  ...
  if (isPlayerBuyer) {
    ...
    const buyerTreasury = Math.max(0, Number(buyerNation.treasury || 0));
    // AI buyers: pay from treasury, capped by a small share of monthly economic capacity.
    ...
    if (price > Math.min(buyerTreasury, maxArmsSpend * (strategy.mode === 'volume' ? 10 : 7))) return false;
    buyerNation.treasury = buyerTreasury - price;
  ...
  if (isPlayerSeller) {
    ...
    sellerNation.treasury = Math.max(0, Number(sellerNation.treasury || 0) + price);
  }
  // Track revenue
  sellerNation.defenseRevenue = (sellerNation.defenseRevenue || 0) + price;

  if (typeof recordNationFinanceFlow === 'function') {
    const itemLabel = meta?.itemName ? `${meta.itemName} x${Math.max(0, Number(meta?.quantity || 0))}` : 'Defense equipment';
    recordNationFinanceFlow(buyerNation, 'outflows', 'arms_purchases', price, {
      note: `Arms import: ${itemLabel}`,
      counterparty: sellerNation.name,
      context: 'defense-market',
    });
    recordNationFinanceFlow(sellerNation, 'inflows', 'arms_sales', price, {
      note: `Arms export: ${itemLabel}`,
      counterparty: buyerNation.name,
      context: 'defense-market',
    });
  }
