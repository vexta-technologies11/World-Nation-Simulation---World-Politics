(function() {
  ...
  const GOAL_LIBRARY = {
    stabilize_prices: {
      ...
      budgetEffect: { economy: 7, diplomacy: 2, military: -4, social: -2, space: -3 },
    ...
    innovation_push: {
      ...
      budgetEffect: { space: 6, intelligence: 4, social: -2, military: -2 },
    ...
    rearmament_drive: {
      ...
      budgetEffect: { military: 8, intelligence: 3, space: -2, social: -4, economy: -2 },
  ...
  function computeBudgetPressure(goals) {
    ...
    goals.forEach(goal => {
      Object.keys(goal.budgetEffect || {}).forEach(key => {
        pressure[key] += safeNum(goal.budgetEffect[key]);
      });
    });
    return pressure;
  }
  ...
  function createAllianceLoan(lender, borrower, alliance, termOverrides) {
    ...
    borrower.debtRatio = clampGov(safeNum(borrower.debtRatio) + amountT * 18, 8, 260);
    invalidateGovernmentReview(lender);
    invalidateGovernmentReview(borrower);
  ...
  function processSingleLoan(loan) {
    ...
    if (paymentT > 0.0001) {
      ...
      borrower.debtRatio = clampGov(safeNum(borrower.debtRatio) - principalPaid * 9, 8, 260);
  ...
  function processSovereignLoanSeeking() {
    ...
    Object.values(NATIONS).forEach(borrower => {
      ...
      // Prefer nations with good relations and high lending capacity
  ...
  function renderInternalAffairsPanel(nationId) {
    ...
    html += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px">';
    ['military', 'economy', 'diplomacy', 'intelligence', 'space', 'social'].forEach(key => {
