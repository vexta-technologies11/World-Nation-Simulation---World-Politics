...
function processCompanyProductEngine(nation) {
  ...
  const cadenceMonths = Math.max(1, Number((typeof window !== 'undefined' && window.ECON_UPDATE_MONTHS) || 1));
  const innovationScore = getCountryInnovationScore(nation);
  ...
  nation.companies.forEach(company => {
    ...
    company.productLines.forEach(line => {
      ...
      line.availableUnits = Math.max(0, Number(line.availableUnits || 0) * 0.62 + Number(line.monthlySupply || 0));
      ...
      line.inputCoverage = effectiveCoverage;
      ...
      totalInputCost += inputResult.inputCost * lineRevenueBase + supplyPurchase.totalCost * 0.55;
      totalImportCost += inputResult.importCost * lineRevenueBase + Math.max(0, supplyPurchase.totalCost * 0.2);
      maxBottleneck = Math.max(maxBottleneck, 1 - effectiveCoverage);
      ...
      supplyPurchase.procurement.forEach(item => {
        nation.productEconomy.imports.push({ companyId: company.id, companyName: typeof getCompanyDisplayName === 'function' ? getCompanyDisplayName(company) : (company.name || 'Company'), productId: line.productId, ...item });
      });
    });
    ...
    if (adjustedRevenue > 0) {
      const blendWeight = cadenceMonths > 1 ? 0.65 : 0.55;
      const blendedRevenue = baselineRevenue * (1 - blendWeight) + adjustedRevenue * blendWeight;
      company.revenue = Math.max(0.05, blendedRevenue);
    }
...
function acquireResourceUnitsForIndustry(nation, resourceId, requestedUnits) {
  let remaining = Math.max(0, Number(requestedUnits || 0));
  let acquired = 0;
  let totalCost = 0;
  const procurement = [];
  ...
  if (localRes) {
    const localCapacity = Math.max(0, Number(localRes.level || 0) * 22);
    const localTake = Math.min(localCapacity, remaining);
    if (localTake > 0) {
      const unitPrice = Math.max(0.08, Number(getResourceTypeDef(resourceId)?.basePrice || 0.5));
      localRes.level = Math.max(0, Number(localRes.level || 0) - localTake / 180);
      localRes.consumed = Number(localRes.consumed || 0) + localTake * 0.01;
      totalCost += localTake * unitPrice;
      acquired += localTake;
      remaining -= localTake;
      procurement.push({ type: 'resource', id: resourceId, from: nation.name || 'domestic market', units: localTake, unitPrice, cost: localTake * unitPrice });
    }
  }
  ...
  if (remaining > 0) {
    ...
    suppliers.forEach(supplier => {
      ...
      if (!res) return;
      const exportCapacity = Math.max(0, (Number(res.level || 0) - 32) * 16);
      if (exportCapacity <= 0) return;
      const take = Math.min(exportCapacity, remaining);
      if (take <= 0) return;
      const unitPrice = Math.max(0.1, Number(getResourceTypeDef(resourceId)?.basePrice || 0.7) * 1.18);
      res.level = Math.max(0, Number(res.level || 0) - take / 220);
      res.consumed = Number(res.consumed || 0) + take * 0.006;
      totalCost += take * unitPrice;
      acquired += take;
      remaining -= take;
      procurement.push({ type: 'resource', id: resourceId, from: supplier?.nation?.name || 'global market', units: take, unitPrice, cost: take * unitPrice });
    });
  }
...
function acquireProductUnitsForIndustry(nation, buyerCompany, productId, requestedUnits) {
  let remaining = Math.max(0, Number(requestedUnits || 0));
  let acquired = 0;
  let totalCost = 0;
  const procurement = [];
  ...
  const localSuppliers = (nation?.companies || [])
    .filter(company => company && company.id !== buyerCompany?.id)
    .map(company => ({ company, nation, local: true }))
  ...
  Object.values(NATIONS || {}).forEach(otherNation => {
    ...
    (otherNation.companies || []).forEach(company => {
      if (!company || company.id === buyerCompany?.id) return;
      const line = (company.productLines || []).find(item => item.productId === productId);
      if (!line || Number(line.availableUnits || 0) <= 0) return;
      globalSuppliers.push({ company, nation: otherNation, local: false });
    });
  });
  ...
  localSuppliers.concat(globalSuppliers).forEach(entry => {
    if (remaining <= 0) return;
    const taken = consumeSupplierProductUnits(entry.company, productId, remaining);
    if (taken <= 0) return;
    const unitPrice = baseUnitPrice * (entry.local ? 1.0 : 1.14);
    const cost = taken * unitPrice;
    entry.company.supplyChainSales = Math.max(0, Number(entry.company.supplyChainSales || 0) + cost);
    entry.company.cashReserves = Math.max(0, Number(entry.company.cashReserves || 0) + cost * 0.32);
    totalCost += cost;
    acquired += taken;
    remaining -= taken;
    procurement.push({
      ...
      id: productId,
      from: entry.local
        ? (typeof getCompanyDisplayName === 'function' ? getCompanyDisplayName(entry.company) : (entry.company.name || 'Local Supplier'))
...
function acquireCompanyInputMaterials(nation, company, productDef, outputTarget) {
  let requiredTotal = 0;
  let acquiredTotal = 0;
  let totalCost = 0;
  const procurement = [];
  ...
  (productDef?.inputs || []).forEach(input => {
    const needed = Math.max(0, Number(outputTarget || 0) * Math.max(0.02, Number(input.intensity || 0.1)) * 1.1);
    if (needed <= 0) return;
    requiredTotal += needed;
    const result = input.type === 'product'
      ? acquireProductUnitsForIndustry(nation, company, input.id, needed)
      : acquireResourceUnitsForIndustry(nation, input.id, needed);
  ...
  return {
    coverage: requiredTotal <= 0 ? 1 : clamp(acquiredTotal / requiredTotal, 0.05, 1),
    totalCost,
    procurement,
  };
}
